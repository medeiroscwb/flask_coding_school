...
#                {"courseID":"3333","title":"Adv PHP 201","description":"Advanced PHP Programming","credits":3,"term":"Fall, Spring"}, 
...
@api.route("/api/<idx>")
class GetGetUpdateDelete(Resource):
    #get one
    def get(self,idx):
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and user.get_password(password):
            ...
            flash("Unable to login, please, verify your email and password.", category="danger")
...
def enrollment():
    ...
    if courseID:
        if Enrollment.objects(user_id=user_id,courseID=courseID):
            flash(f"Oops! You are already registered in this course {courseTitle}.", "warning")
            return redirect(url_for("courses"))
        else:
            Enrollment(user_id=user_id, courseID=courseID).save()
            print(user_id,courseID)
